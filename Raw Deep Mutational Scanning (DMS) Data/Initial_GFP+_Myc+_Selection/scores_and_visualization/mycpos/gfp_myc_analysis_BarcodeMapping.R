...
escape_counts <- read_tsv("/home/adkeith@Eu.Emory.Edu/DMS_Workflow/nucleocapsid/R_p21269_gfp_myc_analysis/exp02_mycpos/sp_nwt_escape.txt")
input_counts <- read_tsv("/home/adkeith@Eu.Emory.Edu/DMS_Workflow/nucleocapsid/R_p21269_gfp_myc_analysis/exp02_mycpos/sp_nwt_input.txt")
...
################################################################################
#Escapees
average_escape$escape <- 1/average_escape$escape
...
ggsave(filename = paste(antibody, "_color_average_escape_01.png", sep=""), 
       ggplot(average_escape1, aes(site, escape, fill = level)) + 
         geom_bar(stat="identity") + 
...
average_escape2 <- subset(average_escape, site>start & site<end)
ggsave(filename = paste(antibody, "_color_average_escape_02.png", sep=""), 
       ggplot(average_escape2, aes(site, escape, fill = level)) + 
         geom_bar(stat="identity") + 
...
################################################################################
#Define the epitope as any residue with average escape more than x SD away from mean
#Approximate the background to a normal distribution
norm_average_cutoff <- quantile(average_escape$escape, probs = .90, na.rm = T)
m_average <- mean(average_escape$escape[average_escape$escape < norm_average_cutoff], na.rm=T)
s_average <- sd(average_escape$escape[average_escape$escape < norm_average_cutoff], na.rm=T)

escapees <- subset(average_escape,escape > (m_average + 4*s_average))
