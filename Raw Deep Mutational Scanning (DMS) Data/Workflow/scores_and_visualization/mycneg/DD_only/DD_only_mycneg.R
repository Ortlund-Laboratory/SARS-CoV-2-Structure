...
escape_counts <- read_tsv("/home/adkeith@Eu.Emory.Edu/DMS_Workflow/nucleocapsid/R_N-05_Myc_Mar_2022_version_to_use_for_paper/exp3_mycneg/DD_only/sp_nwt_escape.txt")
input_counts <- read_tsv("/home/adkeith@Eu.Emory.Edu/DMS_Workflow/nucleocapsid/R_N-05_Myc_Mar_2022_version_to_use_for_paper/exp3_mycneg/DD_only/sp_nwt_input.txt")
...
ggsave(filename = paste(antibody, "_color_average_escape.png", sep=""), 
       ggplot(average_escape, aes(site, escape, fill = level)) + 
         geom_bar(stat="identity") + 
         geom_segment(aes(x=256,xend=365,y=cut_1, yend = cut_1), linetype = 3, size = 0.15) +
         geom_segment(aes(x=256,xend=365,y=cut_2, yend = cut_2), linetype = 3, size = 0.15) +
